...
const Navbar = () => {
  ...
  return (
    <nav className="fixed top-2 z-50 w-screen px-18">
        <div className="container flex items-center justify-between rounded-lg bg-black py-3">
            <div className="flex flex-shrink-0 items-center justify-between">
                ...
                height={30} 
                alt="Logo"/>
                <span className="text-sm tracking-tight text-white">Gojo Felagi</span> 
            </div>
            ...
            <ul className="flex flex-col items-center">
              {NAV_LINKS.map((item,index)=>(
                <li key={index} className="py-6">
                  <Link className="text-sm text-white hover:text-netural-500"href={item.url}>
                  {item.title}
                  </Link>
                </li>
              ))}
